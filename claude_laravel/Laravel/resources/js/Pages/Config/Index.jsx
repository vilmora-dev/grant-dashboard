...
function InitiativesTab({ initiatives, keywords, onRefresh }) {
    ...
    const handleCreateIni = async () => {
        ...
        try {
            const created = await apiCall('POST', '/initiatives', {
    ...
    const handleToggleActive = async (ini) => {
        ...
        try {
            await apiCall('PATCH', `/initiatives/${ini.id}`, { is_active: next })
        } catch (e) {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await apiCall('DELETE', `/initiatives/${target.id}`)
    ...
    const handleCreateKw = async () => {
        ...
        try {
            const created = await apiCall('POST', '/keywords', {
    ...
    const toggleKwActive = async (kw) => {
        ...
        try {
            await apiCall('PATCH', `/keywords/${kw.id}`, { is_active: next })
        } catch (e) {
    ...
    const handleDeleteKw = async (id) => {
        ...
        try {
            await apiCall('DELETE', `/keywords/${id}`)
        } catch (e) {
    ...
    const kwCountMap = {}
    localKeywords.forEach(k => { kwCountMap[k.keyword] = (kwCountMap[k.keyword] || 0) + 1 })
        ...
        <div>
            ...
            <div className="overflow-x-auto rounded-xl border border-[#b2d8d8]">
                <table className="w-full text-[12px] font-sans">
                    ...
                    <tbody>
                        {filteredKw.map(kw => {
                            const sharedCount = kwCountMap[kw.keyword] || 1
                            const ini = iniById[kw.initiative_id]
                            return (
                                <tr key={kw.id} className="border-t border-[#b2d8d8] hover:bg-[#def2f1]/50 transition-colors">
                                    <td className="px-4 py-2.5 text-[#0d2b2b] font-mono text-[11px]">{kw.keyword}</td>
                                    <td className="px-4 py-2.5">
                                        {ini ? (
                                            sharedCount > 1
                                                ? <span className="px-1.5 py-0.5 rounded text-[10px] font-mono bg-[#5a9090]/10 border border-[#5a9090]/20 text-[#5a9090]">×{sharedCount} initiatives</span>
                                                : <span className="px-1.5 py-0.5 rounded text-[10px] font-mono bg-[#3aafa9]/8 border border-[#3aafa9]/20 text-[#2b6e6b]">{ini.slug}</span>
                                        ) : <span className="text-[#8ec8c7] text-[10px]">—</span>}
                                    </td>
                                    <td className="px-4 py-2.5 text-[#2b6e6b]">{kw.priority}</td>
                                    <td className="px-4 py-2.5">
                                        {kw.success_score != null
                                            ? <span className="font-mono text-[11px] text-[#3aafa9]">{Number(kw.success_score).toFixed(1)}</span>
                                            : <span className="text-[#8ec8c7] text-[10px]">—</span>}
                                    </td>
                                    <td className="px-4 py-2.5">
                                        <button onClick={() => toggleKwActive(kw)} className="text-[#2b6e6b] hover:text-[#3aafa9] transition-colors">
                                            {kw.is_active ? <ToggleRight size={18} className="text-[#3aafa9]" /> : <ToggleLeft size={18} />}
                                        </button>
                                    </td>
                                    <td className="px-4 py-2.5">
                                        <button onClick={() => handleDeleteKw(kw.id)} className="text-[#5a9090] hover:text-[#d93050] transition-colors p-1">
                                            <Trash2 size={13} />
                                        </button>
                                    </td>
                                </tr>
...
function OrgProfileTab({ orgProfile }) {
    ...
    const handleSave = async () => {
        ...
        try {
            await apiCall('PUT', '/organization', {
                name:            form.name.trim(),
